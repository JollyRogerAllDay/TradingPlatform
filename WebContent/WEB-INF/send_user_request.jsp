	...
	<body>
		<div id="main">
			...
			<div id="main_content"> 
						...
						<c:choose>
							<c:when test="${sessionScope.permissions == 'SHAREHOLDER'}">
								...
								<div class="nav_menu">
										...
										<br/>
										<a href="send_user_request">Contact System Admin</a>
								</div>
							</c:when>
				...
				<div id="contentarea">
					<h1>Send Request to System Admin</h1>
					<form id="form1" method="post" action="ProcessUserRequestServlet">
					Request:
					<select name="type">
						<c:forEach var="request" items="${permissions }">
							
							<option value=${request }>${request }</option>
						</c:forEach>
					</select>
					<Button id="submit_button" class="button" onmouseover="mouseOverButton(this);" onmouseout="mouseOutButton(this);">Submit</Button>
				</form>
				...
				<c:choose>
					...
					<c:otherwise>
						<p>Request was unable to be sent.</p>
					</c:otherwise>
				</c:choose>
				</div>
			</div>
